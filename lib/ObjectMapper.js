...
ObjectMapper.prototype.map = function(options) {
    ...
    // Get required fields
    var requiredFields = { };
    Object.getOwnPropertyNames(object).forEach(function(fieldName){
        var fieldData = object[fieldName];
        if (fieldData.required) requiredFields[fieldName] = true;
    });
    ...
    // Now check if all required fields are filled
    if (Object.getOwnPropertyNames(requiredFields).length > 0) {
        var missing = '';

        // Stringify missing parameters
        Object.getOwnPropertyNames(requiredFields).forEach(function(fieldName){
            missing += fieldName + ', ';
        });

        missing = missing.substring(0, missing.length - 2);

        deferred.reject([ 'Missing required parameters: ' + missing ]);
    }
...
ObjectMapper.prototype.middleware = function(req, res, next) {
    if (req.query._objectType) {
        ...
        self.map({
            ...
            res.write('Field validation failed due to:\r\n');
            for (var errIndex in errors) {
                console.error('-> ' + errors[errIndex]);
                res.write('-> ' + errors[errIndex] + "\r\n");
            }
